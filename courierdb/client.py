...
class CollectionClient(Generic[T]):
    ...
    def upsert(self, record: T, vector: Optional[List[float]] = None) -> str:
        ...
        if resp.status_code != 200:
            raise CourierDBError(f"Upsert failed: {resp.text}")
    ...
    def read(self, key: str) -> Optional[T]:
        ...
        if resp.status_code == 404:
            return None
        if resp.status_code != 200:
            raise CourierDBError(f"Read failed: {resp.text}")
    ...
    def list(self, limit: int = 100, skip: int = 0) -> List[T]:
        ...
        if resp.status_code != 200:
            raise CourierDBError(f"List failed: {resp.text}")
    ...
    def delete(self, key: str) -> bool:
        ...
        if resp.status_code == 404:
            return False
        if resp.status_code != 200:
            raise CourierDBError(f"Delete failed: {resp.text}")
        return True


class CourierDB:
    def __init__(self, url: Optional[str] = None, api_key: Optional[str] = None):
        self.base_url = url or os.getenv("COURIERDB_URL", "http://localhost:8000")
        self.api_key = api_key or os.getenv("COURIERDB_API_KEY")
    ...
    def list_collections(self) -> List[str]:
        ...
        if resp.status_code != 200:
            raise CourierDBError(f"Failed to list collections: {resp.text}")
